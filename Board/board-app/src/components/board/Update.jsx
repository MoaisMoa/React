...
const Update = () => {
  ...
  const { updateBoard, deleteFile, deleteFiles, isUpdating } = useBoardMutations(id)
  ...
  const onSubmit = (data) => {
    ...
    // 첨부 파일
    if( data.files?.length ) { Array.from(data.files).forEach((f) => formData.append('files', f))}
          ...
          <div className="flex-1">
            {
              preview ? (
                ...
                <div
                  ...
                  <span className='text-xs text-gray-400'>클릭하거나 파일을 드래그하세요</span>
